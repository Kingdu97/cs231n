...
class ConvolutionBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.layers = nn.Sequential(            # 이 self.layers가 CNN 하나의 Block
            nn.Conv2d(in_channels, out_channels, (3, 3), padding=1), # 3*3에 패딩 1이면 입출력 사이즈 똑같
            nn.ReLU(),
            nn.BatchNorm2d(out_channels),
            nn.Conv2d(out_channels, out_channels, (3, 3), stride=2, padding=1), # output사이즈 반으로 줄어듬
            nn.ReLU(),
            nn.BatchNorm2d(out_channels),
        )
